...
if (-not [string]::IsNullOrWhiteSpace($StartAt)) { $body.startAt = $StartAt.Trim() }
if (-not [string]::IsNullOrWhiteSpace($EndAt)) { $body.endAt = $EndAt.Trim() }

$uri = "$($ApiBase.TrimEnd('/'))/ops/entitlements/grant"
